...
Perus-GAM, jossa random-rakenne on smootherina. 
...
*4.7.2 Including a random slope* In similar fashion, we may include a by-speaker linear random slope (which would
correspond to tilting the non-linear pattern) for the two-word-contrast (Word) as follows:  

m4 <- bam(Pos ~ Word + s(Time, by=Word) + s(Speaker,bs="re") + s(Speaker,Word,bs="re"), data=dat)  
 
In the lmer specification this random slope would be represented by **(0+Word|Speaker)**. Unfortunately, in the GAM specification, it is not possible to model a
correlation between random intercepts and random slopes (i.e. an lmer specification such as (1+Word|Speaker) is not possible). At present this is a drawback compared to linear
mixed-effects regression, at least when linear random slopes are used.

Eli alla käyttämäni random-rakenne ei ole oikein, tai ainakaan just se mitä haetaan. Pitäisi kai käyttää GAMM-funktiota sit kuitenkin. 

```{r}
M0 <- gam(terri ~ year + s(year,id, bs ="re"), offset=log(area), family = nb(), data= dataS, method = "ML")

M1 <- gam(terri ~ SSS + agri5*year + s(Y, bs ="cs") + s(X, bs ="cs") + s(year,id, bs="re"), offset=log(area), family = nb(),data= dataS, method = "ML")

M2 <- gam(terri ~ SSS + agri5*year + s(Y, bs ="cs") + s(X, bs ="cs") + year*temp + year*prec + s(year,id, bs="re"), offset=log(area), family = nb(), data= dataS, method = "ML")

M3 <- gam(terri ~ SSS + agri5*year + s(Y, bs ="cs") + s(X, bs ="cs") + year*shanG + year*open + s(year,id, bs="re"), offset=log(area), family = nb(), data= dataS, method = "ML")

M4 <- gam(terri ~ SSS + agri5*year + s(Y, bs ="cs") + s(X, bs ="cs") + year*temp + year*prec + year*shanG + year*open + s(year,id, bs="re"), offset=log(area), family = nb(), data= dataS, method = "ML")

AIC(M0,M1,M2,M3,M4)
```

M2 ja M4 ovat parhaimpia näistä. M2 on yksinkertaisempi.
...
M6 <- gam(terri ~ SSS + agri5*year + s(Y) + s(X) + year*temp + year*prec + year*shanG + year*open + offset(log(area)) + s(year,id, bs =  "re"), family = nb(),data = dataS)
summary(M6)
Theta <- M6$family$getTheta(TRUE)


# If you want then you can run M4 again with the optimal theta.
M4 <- gamm4(terri ~ SSS + agri5*year + s(Y) + s(X) + year*temp + year*prec + year*shanG + year*open + offset(log(area)),
            random = ~(1 + year|id),
...
Malli G4 ilmoittaa "failure to converge in 10000 evaluations" mutta tämä ei kuitenkaan ole mikään error-viesti ja malli näyttää toimineen. Jännä juttu. Mulla on R:ssä asennettuna joku paketti, joka kertoo laskelmien edistymisestä. Teksti taitaa tulla sen takia. Selvitetään..
...
G3 ja G5 näyttävät saavan pienimmät AIC arvot. Ero näiden välillä on hyvin pieni. 
Mutta joissain esimerkeissä vertaillaan BIC arvoja, näissä mitattuna G2 olisi paras.

Entäs kun ero on noin mitättömän pieni? Valitaanko yksinkertaisempi malli? G3?
...
Tää validointi on Zuurin esimerkistä GAMM NB esimerkistä. En vielä osaa sen kummemmin tulkita näitä. Enkä tiedä mitä kaikkea pitäisi plotata.


Samat mallit voisi tehdä vielä niin, että SSS on eroteltu Abuild + Astream + Aroad + Ariver. Kokeilen tätä sitten myöhemmin kun päädyn johonkin ratkaisuun mallin rakenteesta ja funktiosta.


#### 2.2.6 gamm-funktio

Samat mallit voisi ajaa myös gamm-funktiolla mutta miten tehdään mallin vertailu jos AIC ei ole käytettävissä? Paitsi, että Zuurin kirjassa AIC vertailua tehdään kyllä ihan mennen tullen. 
...
```
Hmm.. Tämän mukaan malli G4 on paras. Selvällä erolla.
...
X ja Y ei ehkä tarpeellisia jos tulee alueen identifioiva muuttuja. 
