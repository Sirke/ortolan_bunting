...
Miten gamm malleja vertaillaan? AIC ei kai ihan pätevä. gamm4 ja gamm funktioilla eroa, gamm ei anna AIC arvoa. katso Zuurin pingviiniesimerkit.
...
##########################################################################
M6 <- gam(NCalls ~ (terri ~ SSS + agri5*year + s(Y) + s(X) + year*temp + year*prec + year*shan + year*open + offset(log(area))+
                   s(id, bs =  "re"), #miten tähän laitetaan random intercept ja slope??
          family = nb(),
          data = Owls)
summary(M6)
...
M4 <- gamm4(NCalls ~ SexParent * FoodTreatment +
              s(ArrivalTime) +
              offset(LogBroodSize),
            random =~ (1 | Nest),
...
```{r, eval=FALSE}
G1=gamm(terri ~ year, offset=log(area), random= list(id=~1+year), family=nbinom2, data=dataS) #oisko tämä syntaksi oikein?
