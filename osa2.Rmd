...
formula (this used to conform to the behaviour of lm and glm)." Eli jos argumenttina niin predicted arvot eivät huomioi

```{r}
#joku smootheri mallissa täytyy olla
#vai pitäisikö perusmallin olla glmm? jos ainut selittävä tekijä on year.
M1=gamm4(terri ~ year + offset(log(area)), random= ~(1 + year|id), family=nbinom2, data=dataS)


M2=gamm4(terri ~ SSS + agri5*year + s(Y) + s(X) + year + offset(log(area)) , random= ~(1 + year|id), family=nbinom2, data=dataS)
